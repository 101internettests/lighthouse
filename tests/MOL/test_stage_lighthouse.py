import subprocess
from docs.urls_path import path_MOL_stage, websites_to_check_MOL_stage


def test_run_lighthouse():
    for i, website_to_check in enumerate(websites_to_check_MOL_stage):
        output_path = f"{path_MOL_stage[i].replace('/', '_')}.html"  # Using path names for the output
