import subprocess
from docs.urls_path import websites_to_check_stage, path_stage


def test_run_lighthouse():
    for i, website_to_check in enumerate(websites_to_check_stage):
        output_path = f"{path_stage[i].replace('/', '_')}.html"  # Using path names for the output
